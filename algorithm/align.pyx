...
* change floats to ints (make correction for large numbers first)
...
def align_pairwise(
        list seqA, # sequenceA
        list seqB, # sequenceB
        list wghA, # weightsA == gap scores
        list wghB, # weightsB == gap scores
        list resA, # restrictions
        list resB, # restrictions
        str prsA, # prosodic string
        str prsB, # prosodic string
        dict score_dict, # 
        float scale,
    ...
    # here we create the scorer, a matrix which gives scores for all matchings
    cdef list scorer = [[0 for i in range(lA+1)] for j in range(lB+1)]
    
    # set lines[0] of scorer to gap weights
