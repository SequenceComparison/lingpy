...
from algorithm.cluster import upgma
...
class Multiple(_Multiple):
    ...
    *msa-format*
    ...
    def _init_msa(
        ...
        for i,line in enumerate(self.alm_matrix):
            for j,cell in enumerate(line):
                self.alm_matrix[i][j] = unicode(cell,'utf-8')
    ...
    def output(
        ...
        # length
        mtax = max([len(unicode(t,'utf-8')) for t in self.taxa])
        txf = '{0:.<'+str(mtax)+'}'
        ...
        if not sorted_seqs or fileformat == 'psa':
            for i,taxon in enumerate(self.taxa):
                if fileformat == 'msq':
                    out.write(txf.format(taxon)+'\t'+self.seqs[i].encode('utf-8')+'\n')
                elif fileformat == 'msa':
                    out.write(txf.format(taxon)+'\t')
                    out.write('\t'.join(self.alm_matrix[i]).encode('utf-8')+'\n')
                elif fileformat == 'psa':
                    ...
                    for j,taxonB in enumerate(self.taxa):
                        if i < j:
                            ...
                            out.write('{0} ({1}, {2})\n'.format(
                                ...
                                taxonB))
                            out.write(txf.format(taxon)+'\t')
                            out.write('\t'.join(almA).encode('utf-8')+'\n')
                            out.write(txf.format(taxonB)+'\t')
                            out.write('\t'.join(almB).encode('utf-8')+'\n')
                            out.write('{0} {1:.2f}\n\n'.format(
                                self.comment,
                                score))
        elif sorted_seqs:
            if fileformat == 'msa':
                alms = ['\t'.join(alm).encode('utf-8') for alm in self.alm_matrix]
            else:
                alms = [seq.encode('utf-8') for seq in self.seqs]
            
            if not unique_seqs:
                taxalms = zip(self.taxa,alms)
                if sorted_seqs == 'taxa':
                    taxalms = sorted(taxalms,key=lambda x:x[0].lower())
                elif sorted_seqs == 'tree':
                    # calculate upgma-tree
                    # get the matrix
                    matrix = []
                    uniseqs = sorted(self.uniseqs.keys())
                    for i,seqA in enumerate(uniseqs):
                        for j,seqB in enumerate(uniseqs):
                            if i < j:
                                almA = self.alm_matrix[self.uniseqs[seqA][0]]
                                almB = self.alm_matrix[self.uniseqs[seqB][0]]
                                matrix.append(1.0-pid(almA,almB))
                    ...
                    tree = sub(
                            ...
                            upgma(
                                matrix,
                                [seq.encode('utf-8') for seq in uniseqs],
                                distances=False
                                )
                            )
                    tree = [unicode(t,'utf-8') for t in tree.split(',')]
                    taxalms = []
                    for t in tree:
                        taxa = [self.taxa[x] for x in self.uniseqs[t]]
                        alm = self.alm_matrix[self.uniseqs[t][0]]
                        
                        for taxon in taxa:
                            taxalms.append((taxon,'\t'.join(alm).encode('utf-8')))
                ...
                if sorted_seqs == 'taxa':
                    taxalms = sorted(taxalms,key=lambda x:x[0])
                elif sorted_seqs == 'tree':
                    # calculate upgma-tree
                    # get the matrix
                    matrix = []
                    ...
                    taxalms = []
                    for t in tree:
                        taxon = self.taxa[self.uniseqs[t][0]]
                        alm = self.alm_matrix[self.uniseqs[t][0]]
        ...
        if fileformat == 'msa':
            if hasattr(self,'peak_idx'):
                if self.peak_idx and len(self.peak_idx) == len(self.alm_matrix):
...
class Pairwise(_Pairwise):
    ...
    *psa-format*
        ...
        while i <= len(data) - 3:
            try:
                ...
                self.alignments.append(
                        (
                            [unicode(a,'utf-8') for a in almA],
                            [unicode(b,'utf-8') for b in almB],
                            0)
                        )
                i += 4
            except:
    ...
    def output(
        ...
        local : bool
            Determine whether the data in the psa-file should be in compact,
            local format, i.e. only with internal gaps, or not.
        ...
        # check whether outfile already exists -- this is discarde for the
        # moment, since it didn't turn out to work very nicely...
        #try:
        #    tmp = open(outfile)
        #    tmp.close()
        #    outfile = filename + '_out.' + fileformat
        ...
        # if data is psa-format
        elif fileformat == 'psa' and not local:
            out.write(self.dataset + '\n')
            for i,(a,b,c) in enumerate(self.alignments):
                ...
                out.write(txf.format(self.taxa[i][0])+'\t'+'\t'.join(a).encode('utf-8')+'\n')
                out.write(txf.format(self.taxa[i][1])+'\t'+'\t'.join(b).encode('utf-8')+'\n')
                out.write('{0} {1:.2f}'.format(self.comment,c)+'\n\n')
            out.close()

        # if data is psa-format
        elif fileformat == 'psa' and local:
            out.write(self.dataset + '\n')
            for i,(a,b,c) in enumerate(self.alignments):
                out.write(self.seq_ids[i]+'\n')
                
                # determine ungapped internal subsequence
                zips = zip(a,b)
                pos = []
                # start checking from the left
                for j,(x,y) in enumerate(zips):
                    if '-' in [x,y]:
                        pos.append(j)
                    else:
                        break
                # check from the right
                for j,(x,y) in list(enumerate(zips))[::-1]:
                    if '-' in [x,y]:
                        pos.append(j)
                    else:
                        break

                # get new alignments for a and b
                newA = [a[j] for j in range(len(a)) if j not in pos]
                newB = [b[j] for j in range(len(b)) if j not in pos]
                ...
                out.write(txf.format(self.taxa[i][0])+'\t'+'\t'.join(newA).encode('utf-8')+'\n')
                out.write(txf.format(self.taxa[i][1])+'\t'+'\t'.join(newB).encode('utf-8')+'\n')
                out.write('{0} {1:.2f}'.format(self.comment,c)+'\n\n')
            out.close()
